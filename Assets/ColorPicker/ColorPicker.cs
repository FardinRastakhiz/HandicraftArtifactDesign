...
public class ColorPicker : MonoBehaviour {
	...
	public static bool useExternalDrawer = false;
    ...
    [SerializeField]
    Fardin.ColorTools.ColorTerminal terminal;
    ...
    RectTransform newUIRect;
    bool starting;
	void Start()
	{
        starting = true;
        newUIRect = gameObject.GetComponent<RectTransform>();
		sizeCurr = sizeHidden;
	...
	void OnGUI()
    {
        if (terminal.colorForm.isChanged)
        {
            SetColor(terminal.colorForm.RGB);
            ApplyColor();
        }
        ...
        {
            sizeHidden = (int)((newUIRect.rect.width * newUIRect.lossyScale.y)/1.4f);
            if (starting)
            {
                sizeCurr = sizeHidden;
                starting = false;
            }
            startPos.x = newUIRect.position.x - (newUIRect.rect.width * newUIRect.lossyScale.y / 2) + 0.2f * sizeHidden;
            startPos.y = Screen.height - newUIRect.position.y - (newUIRect.rect.height * newUIRect.lossyScale.y / 2) + 0.1f * sizeHidden;
            GUI.DrawTexture(new Rect(startPos.x + sizeHidden + 14 * newUIRect.lossyScale.y, startPos.y, sizeHidden, sizeHidden), txColorDisplay);
			_DrawGUI();
		}
	}
	...
	public void _DrawGUI () 
	{
        ...
        if (mState == ESTATE.Hidden && overalChange)
        {
            sizeCurr = Mathf.Lerp(sizeFull, sizeHidden, dt / animTime);
            if (dt / animTime > 1.0f)
            {
		...
		//draw color picker
        Rect rect = new Rect(startPos.x, startPos.y - (sizeCurr - sizeHidden), sizeCurr, sizeCurr);
		GUI.DrawTexture(rect, colorSpace);

		//Vector2 startPosAlpha = startPos + new Vector2(0, sizeCurr);
        //Rect rectAlpha = new Rect(startPosAlpha.x, startPosAlpha.y - (sizeCurr - sizeHidden + alphaGradHeight), sizeCurr, alphaGradHeight);
		...
		if(mState == ESTATE.Showed)
		{
			if(rect.Contains(e.mousePosition))
			{
                ...
                Vector2 localImagePos = (mousePos - new Vector2(startPos.x, startPos.y - (sizeCurr - sizeHidden)));
	...
	{
		SelectedColor = TempColor;
        ColorTools.SetSelectionsColor(SelectedColor);
	}
}
